...
int ePrestamo_cantidadDias(ePrestamo prestamos[],int tam)
{
    ...
    for(i=0;i<tam-1;i++)
    {
        for(j=i+1;j<tam;j++)
        {
            if((prestamos[i].estado==1 && prestamos[j].estado==1)&& (prestamos[i].fechaPrestamo.dia!=prestamos[j].fechaPrestamo.dia || prestamos[i].fechaPrestamo.mes!=prestamos[j].fechaPrestamo.mes || prestamos[i].fechaPrestamo.anio!=prestamos[j].fechaPrestamo.anio))
            {
                i=j;
                fechas++;
            }
...
void ePrestamo_listarCantidadDiasNoSuperanPromedio(ePrestamo prestamos[], int tam)
{
    ...
    }else
    {
        for(i=0;i<tam-1;i++)
        {
            for(j=i+1;j<tam;j++)
            {
                if(prestamos[i].estado==1 && prestamos[j].estado==1)
                {
        ...
        if((prestamos[tam-1].estado==1 && prestamos[tam-2].estado==1) && (prestamos[tam-1].fechaPrestamo.dia!=prestamos[tam-2].fechaPrestamo.dia || prestamos[tam-1].fechaPrestamo.mes!=prestamos[tam-2].fechaPrestamo.mes || prestamos[tam-1].fechaPrestamo.anio!=prestamos[tam-2].fechaPrestamo.anio)&&(promedio>1))
        {
            noSuperan++;
        }
