...
void eSocio_solicitaronUnLibro(eSocio socios[], int tamSocios, eLibro libros[], int tamLibros,ePrestamo prestamos[],int tamPrestamos)
{
    ...
    if(contador!=0)
    {
        ...
        {
            if(prestamos[i].estado== 1 && (prestamos[i].codigoLibro == codigo))
            {
                for(j=0;j<tamSocios;j++)
                {
                    if((prestamos[i].codigoSocio == socios[j].codigo) && socios[j].estado == 1)
                    {
                        for(k=0;k<tamLibros;k++)
                        {
                            if(libros[k].estado == 1 && libros[k].codigo == prestamos[i].codigoLibro)
                            {
    ...
    if(valida!= 0)
    {
        for (i=0;i<tam;i++)
        {
            if(socios[i].estado == 1)
            {
                ...
                {
                    if (socios[i].codigo==prestamos[j].codigoSocio && prestamos[j].estado == 1)
                    {
                        contador++;
                    }
                }
        ...
        if (flag==1)
        {
            ...
            for (k=0;k<tam;k++)
            {
                if(socios[k].estado == 1)
                {
                    contador=0;
                    for (l=0;l<tamPrestamos;l++)
                    {
                        if (socios[k].codigo==prestamos[l].codigoSocio && prestamos[l].estado == 1)
                        {
                            contador++;
                        }
                    }
    ...
    if(valida!=0)
    {
        ...
        for (i=0;i<tam;i++)
        {
            if(socios[i].estado == 1)
            {
                for (j=0;j<tamPrestamos;j++)
                {
                    if (socios[i].codigo == prestamos[j].codigoSocio && prestamos[j].fechaPrestamo.dia== dia && prestamos[j].fechaPrestamo.mes == mes && prestamos[j].fechaPrestamo.anio == anio && prestamos[j].estado == 1)
                    {
