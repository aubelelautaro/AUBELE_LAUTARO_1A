...
void eLibro_listarDescendiente(eLibro libros[], int tam)
{
    ...
    while (ordenado==1)
    {
        ...
        for (i = 1; i < tam; i++)
        {
            if (strcmp(libros[i].titulo, libros[i- 1].titulo)>0 && libros[i].estado == 1)
            {
                aux = libros[i];
                libros[i] = libros[i - 1];
                libros[i - 1] = aux;
...
void eLibro_solicitadoPorUnSocio(eSocio socios[], int tamSocios, eLibro libros[], int tamLibros,ePrestamo prestamos[],int tamPrestamos)
{
    ...
    if(contador!= 0)
    {
        ...
        {
            if(prestamos[i].codigoSocio == codigo && prestamos[i].estado == 1)
            {
                for(j=0;j<tamLibros;j++)
                {
                    if(libros[j].codigo == prestamos[i].codigoLibro)
                    {
                        ...
                        {
                            if(prestamos[i].codigoSocio == socios[k].codigo && libros[j].estado == 1)
                            {
    ...
    if(contador!=0)
    {
        ...
        }else
        {
            for(k=0;k<tamLibros;k++)
            {
                ...
                for(l=0;l<tam;l++)
                {
                    if(libros[k].codigo == prestamos[l].codigoLibro && prestamos[l].estado == 1 && libros[k].estado == 1)
                    {
                        contadorLibros++;
                    }
                }
...
void eLibro_fechaDeterminada(eLibro libros[], int tam, ePrestamo prestamos[], int tamPrestamos)
{
    ...
    if(valida!=0)
    {
        ...
        for (i=0;i<tam;i++)
        {
            ...
            {
                if (libros[i].codigo == prestamos[j].codigoLibro && prestamos[j].fechaPrestamo.dia== dia && prestamos[j].fechaPrestamo.mes == mes && prestamos[j].fechaPrestamo.anio == anio && prestamos[j].estado == 1)
                {
